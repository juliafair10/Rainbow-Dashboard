function claimFolderMapHasDuplicate_(claimData, thread) {
  ...
  try {
    ...
    const sheet = ss.getSheetByName(INTAKE_CONFIG.claimFolderMapSheetName);
...
function checkOrCreateClaimFolder_(claimData) {
  ...
  try {
    ...
    const rootFolder = DriveApp.getFolderById(
      INTAKE_CONFIG.claimFolderParentFolderId
    );
    ...
    if (!INTAKE_CONFIG.createClaimFolders) {
...
function appendClaimFolderMapRow_(claimData, folderResult, thread) {
  ...
  try {
    ...
    const ss = SpreadsheetApp.openById(INTAKE_CONFIG.claimFolderMapSpreadsheetId);
    let sheet = ss.getSheetByName(INTAKE_CONFIG.claimFolderMapSheetName);

    if (!sheet) {
      sheet = ss.insertSheet(INTAKE_CONFIG.claimFolderMapSheetName);
    }
    ...
    const notes = [
      'Created by ' + INTAKE_CONFIG.automationName,
...
function findExistingClaimFolderByVendorDriveAddress_(candidateAddress, claimData, thread) {
  ...
  try {
    ...
    const rootFolder = DriveApp.getFolderById(INTAKE_CONFIG.claimFolderParentFolderId);
    const currentYear = String(new Date().getFullYear());
