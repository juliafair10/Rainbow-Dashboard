...
function writeAttachmentManifest_() {
  ...
  const result = {
    ...
    manifestEnabled: INTAKE_CONFIG.attachments && INTAKE_CONFIG.attachments.manifestEnabled === true,
    manifestFileId: '',
    manifestFileUrl: '',
    manifestFileName: INTAKE_CONFIG.attachments && INTAKE_CONFIG.attachments.manifestFileName ? INTAKE_CONFIG.attachments.manifestFileName : 'attachment-manifest.json',
    created: false,
    updated: false,
    error: ''
  };
...
function readAttachmentManifest_() {
  ...
  try {
    ...
    const folder = DriveApp.getFolderById(folderId);
    const manifestName = INTAKE_CONFIG.attachments && INTAKE_CONFIG.attachments.manifestFileName ? INTAKE_CONFIG.attachments.manifestFileName : 'attachment-manifest.json';
...
function buildAttachmentManifestPayload_(context) {
  ...
  return {
    schemaVersion: INTAKE_CONFIG.attachments && INTAKE_CONFIG.attachments.manifestSchemaVersion ? INTAKE_CONFIG.attachments.manifestSchemaVersion : 1,
...
function appendAttachmentLogRows_() {
  ...
  if (!INTAKE_CONFIG.attachments || INTAKE_CONFIG.attachments.loggingEnabled !== true) {
    return result;
  }
  ...
  try {
    ...
    let sheet = ss.getSheetByName(INTAKE_CONFIG.attachmentLogSheetName);

    if (!sheet) {
      sheet = ss.insertSheet(INTAKE_CONFIG.attachmentLogSheetName);
    }
...
function copyEligibleAttachments_() {
  ...
  try {
    ...
    if (INTAKE_CONFIG.dryRun || (INTAKE_CONFIG.attachments && INTAKE_CONFIG.attachments.copyEnabled === false)) {
      result.status = INTAKE_CONFIG.dryRun ? 'dry_run_copy_skipped' : 'copy_disabled';
