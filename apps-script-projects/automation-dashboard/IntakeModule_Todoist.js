...
function testTodoistConnection() {
  ...
  const result = {
    ...
    finishedAt: null,
    config: {
      enabled: INTAKE_CONFIG.todoist && INTAKE_CONFIG.todoist.enabled === true,
      hasApiToken: false,
      hasProjectId: false,
      hasAssigneeIdClarence: false,
      hasSectionId: false,
      dueString: INTAKE_CONFIG.todoist && INTAKE_CONFIG.todoist.dueString ? INTAKE_CONFIG.todoist.dueString : '',
      priority: INTAKE_CONFIG.todoist && INTAKE_CONFIG.todoist.priority ? INTAKE_CONFIG.todoist.priority : ''
    },
    todoist: {
      reachable: false,
      projectTaskCount: 0,
...
function getTodoistConfig_() {
  ...
  const todoistConfig = INTAKE_CONFIG.todoist || {};
