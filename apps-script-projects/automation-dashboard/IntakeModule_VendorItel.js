function getItelWorkflowStateLabelsToRemove_() {
  return dedupeLabelNames_([
    INTAKE_CONFIG.itelIntakeLabel,
    INTAKE_CONFIG.itelNeedsReviewLabel,
    INTAKE_CONFIG.itelPendingClaimFolderLabel,
    INTAKE_CONFIG.itelErrorLabel,
...
function processItelAttachments() {
  ...
  try {
    const threads = GmailApp.search(INTAKE_CONFIG.itelQuery, 0, INTAKE_CONFIG.maxThreadsPerRun);
      ...
      try {
        ...
        if (!claimData.claimNumber) {
          ...
          const labelResult = applyInsuranceIntakeLabels_(thread, {
            add: [INTAKE_CONFIG.itelNeedsReviewLabel],
            remove: [INTAKE_CONFIG.itelIntakeLabel]
          });
        ...
        const itelSubfolderResult = checkOrCreateVendorSubfolder_(folderResult.folderId, INTAKE_CONFIG.itelSubfolderName);
        ...
        if (!itelSubfolderResult.success) {
          ...
          const labelResult = applyInsuranceIntakeLabels_(thread, {
            add: [INTAKE_CONFIG.itelErrorLabel],
            remove: [INTAKE_CONFIG.itelIntakeLabel]
          });
        ...
        const labelResult = applyInsuranceIntakeLabels_(thread, {
          add: [INTAKE_CONFIG.itelProcessedLabel],
...
function buildItelAttachmentMessage_(summary) {
  return 'Phase ' + INTAKE_CONFIG.phase + ' Itel attachment intake found ' + summary.foundCount +
